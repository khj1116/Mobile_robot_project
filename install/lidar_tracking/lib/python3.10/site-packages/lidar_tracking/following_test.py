...
class LidarPersonTracking(Node):
    ...
    def lidar_callback(self, msg):
        ...
        # ✅ DBSCAN 클러스터링 수행
        clustering = DBSCAN(eps=0.1, min_samples=18).fit(points)
        labels = clustering.labels_

        # ✅ 사람의 다리 쌍을 인식하여 정확한 사람 추종
        person_position = self.detect_person_legs(points, labels)

        if person_position is not None and self.is_valid_person(person_position):
            self.update_kalman(person_position)
            self.target_position = self.get_kalman_prediction()
            self.last_seen_time = time.time()
            # 속도 업데이트
            if self.training_data:
                delta_time = time.time() - self.last_seen_time + 1e-5
                self.prev_velocity = np.linalg.norm(person_position - self.training_data[-1]) / delta_time
            # KNN 학습 데이터 추가
            self.training_data.append(person_position)
            if len(self.training_data) > 10:  # 최대 10개 데이터 유지
                self.training_data.pop(0)
            self.knn.fit(self.training_data, [0] * len(self.training_data))
            self.knn_fit = True
        else:
            # 대상이 없으면 3초간 예측 유지
    ...
    def detect_person_legs(self, points, labels):
        ...
        min_leg_distance = 0.25
        max_leg_distance = 0.3
        best_pair = None
