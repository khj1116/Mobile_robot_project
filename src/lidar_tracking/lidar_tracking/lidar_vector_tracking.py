# 동작 흐름
# 1. 타겟 감지 시 : position_history 와 velocity_history 를 업데이트
# 2. update_bayesian_prior로 prior 확률 분포 갱신
# 3. 타겟 사라짐 시
# predict_with_bayesian 호출
# 이전 방향과 속도로 예측 위치 계산, 라이다 데이터로 벽/코너 확인 후 확률 조정
# 칼만 필터와 결합하여 최종 위치 예측
# 4. 추적 : 예측된 target_position을 기반으로 로봇 이동
...
class LidarPersonTracking(Node):
    def __init__(self):
        ...
        self.last_scan = None  # LiDAR 데이터 저장
        ...
        # 베이지안 필터용 변수
        self.grid_size = 0.1  # 그리드 해상도 (m)
        self.grid_range = 5.0  # 예측 범위 (m)
        self.prob_grid = None  # 확률 그리드

    def lidar_callback(self, msg):
        self.last_scan = msg  # 최신 LiDAR 데이터 저장
        ...
        kalman_predicted = self.kalman.predict()[:2].flatten()

        if person_position is not None:
            if self.is_valid_person(person_position, kalman_predicted):
                ...
                self.update_bayesian_prior()  # 타겟 감지 시 prior 갱신
            elif self.target_position is not None:
                self.target_position = self.predict_with_bayesian(points)
        else:
            if time.time() - self.last_seen_time < 5.0 and self.target_position is not None:  # 예측 연장 시간 증가
                self.target_position = self.predict_with_bayesian(points)
            else:
                self.target_position = None
                self.prob_grid = None
    ...
    def update_bayesian_prior(self):
        """이전 이동 방향과 속도를 기반으로 prior 확률 분포 갱신"""
        if len(self.position_history) < 2 or len(self.velocity_history) < 1:
            return

        # 그리드 초기화
        grid_dim = int(self.grid_range / self.grid_size) * 2 + 1
        if self.prob_grid is None:
            self.prob_grid = np.ones((grid_dim, grid_dim)) / (grid_dim * grid_dim)
        ...
        # 중심을 마지막 위치로 설정
        center_x = int(grid_dim / 2)
        center_y = int(grid_dim / 2)

        # 이동 방향에 따라 가우시안 분포 적용
        for i in range(grid_dim):
            for j in range(grid_dim):
                x = (i - center_x) * self.grid_size
                y = (j - center_y) * self.grid_size
                dist_along_direction = x * direction[0] + y * direction[1]
                dist_perp_direction = np.abs(x * direction[1] - y * direction[0])
                # 이동 방향에 따른 확률: 속도와 시간 반영
                prob = np.exp(-((dist_along_direction - avg_velocity * 0.1)**2 / (2 * 0.5**2)) - (dist_perp_direction**2 / (2 * 0.2**2)))
                self.prob_grid[i, j] = max(self.prob_grid[i, j], prob)

        self.prob_grid /= np.sum(self.prob_grid)  # 정규화

    def predict_with_bayesian(self, points):
        """베이지안 필터로 다음 위치 예측"""
        if self.prob_grid is None or len(self.position_history) < 2 or self.last_scan is None:
            return self.kalman.predict()[:2].flatten()

        # 환경적 요인 반영: 벽과 코너 감지
        angles = np.linspace(self.last_scan.angle_min, self.last_scan.angle_max, len(self.last_scan.ranges))
        ranges = np.array(self.last_scan.ranges)
        x = ranges * np.cos(angles)
        y = ranges * np.sin(angles)
        valid = ~np.isnan(x) & ~np.isnan(y)
        env_points = np.column_stack((x[valid], y[valid]))

        grid_dim = self.prob_grid.shape[0]
        center_x = int(grid_dim / 2)
        center_y = int(grid_dim / 2)
        last_pos = self.position_history[-1]
        ...
        for i in range(grid_dim):
            for j in range(grid_dim):
                pred_x = last_pos[0] + (i - center_x) * self.grid_size
                pred_y = last_pos[1] + (j - center_y) * self.grid_size
                for env_point in env_points:
                    dist_to_env = np.linalg.norm([pred_x - env_point[0], pred_y - env_point[1]])
                    if dist_to_env < 0.2:  # 벽 근처일 경우 확률 감소
                        self.prob_grid[i, j] *= 0.1

        self.prob_grid /= np.sum(self.prob_grid)  # 재정규화

        # 최대 확률 위치 계산
        max_prob_idx = np.argmax(self.prob_grid)
        max_i, max_j = np.unravel_index(max_prob_idx, self.prob_grid.shape)
        predicted_x = last_pos[0] + (max_i - center_x) * self.grid_size
        predicted_y = last_pos[1] + (max_j - center_y) * self.grid_size

        # 칼만 필터와 결합
        kalman_pred = self.kalman.predict()[:2].flatten()
        alpha = 0.7  # 베이지안 예측 가중치
        combined_pred = alpha * np.array([predicted_x, predicted_y]) + (1 - alpha) * kalman_pred

        self.get_logger().info(f"Bayesian predicted position: {combined_pred}")
